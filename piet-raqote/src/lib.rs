...
impl<'a> RenderContext for RaqoteRenderContext<'a> {
    ...
    match format {
      ...
      ImageFormat::RgbaPremul => {
        for i in buf.chunks(4) {
          image.push(
            ((i[3] as u32) << 24) | ((i[0] as u32) << 16) | ((i[1] as u32) << 8) | (i[2] as u32),
          )
        }
      }
      ImageFormat::RgbaSeparate => {
        ...
        for i in buf.chunks(4) {
          let a = i[3];
          image.push(
            ((a as u32) << 24) | (premul(i[0], a) << 16) | (premul(i[1], a) << 8) | premul(i[2], a),
          )
        }
      }
  ...
  fn draw_image(&mut self, image: &Self::Image, rect: impl Into<Rect>, _interp: InterpolationMode) {
    let rect = rect.into();
